...
auto LibFlute::FileDeliveryTable::add(const FileEntry& fe) -> bool
{
  ...
  /* Content-Type is required of the sender under either 3GPP profile, so an entry that carries
     ...
     source for the substitution and rule 12 prefers failing loudly. A caller with genuinely typeless
     octets can say so explicitly with set_content_type("application/octet-stream"), which records
